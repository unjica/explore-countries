...
let lightSwitcher = ref("");
let darkMode = ref(localStorage.getItem('dark-mode') === 'true');
...
<template>
  <div class="w-fit">
    <input v-model="darkMode" ref="lightSwitcher" type="checkbox" name="lightSwitcher" id="lightSwitcher" class="hidden" />
    <label for="lightSwitcher" class="flex items-center justify-center cursor-pointer w-8 h-8 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600/80 rounded-full">
